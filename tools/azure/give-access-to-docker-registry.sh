...
while [[ "$#" -gt 0 ]]; do
    case "$1" in
        ...
        --docker-registry-name) AZURE_RESOURCE_NAME="$2"; shift ;;
...
if [[ -z "$AZURE_SUBSCRIPTION_ID" || -z "$AZURE_RESOURCE_GROUP" || -z "$AZURE_RESOURCE_NAME" || -z "$AZURE_CLIENT_ID" ]]; then
    echo "Usage: $0 --subscription-id AZURE_SUBSCRIPTION_ID --resource-group AZURE_RESOURCE_GROUP --docker-registry-name DOCKER_REGISTRY_NAME --client-id AZURE_CLIENT_ID"
    exit 1
fi
